...
from init_cond_opt import init_opt
...
def build_Q(contact_vecs:np.ndarray):
    Q_list = np.zeros((n_contacts,(ke+1),(ke+1)))
    for i in range(n_contacts):
        Qi = np.zeros(((ke+1),(ke+1)))
        cone_vecs_i = contact_vecs[(ke+1)*i:(ke+1)*i+(ke+1),:]
        norm_i = cone_vecs_i[0,:]
        for j in range(ke+1):
            for k in range(ke+1):
                    Qi[k,j] = np.dot(cone_vecs_i[j,:].T,cone_vecs_i[k,:])

        Q_list[i,:,:] = Qi
    
    Q = block_diag(Q_list[0,:,:],Q_list[1,:,:],Q_list[2,:,:],Q_list[3,:,:],Q_list[4,:,:])

    assert np.allclose(Q,Q.T), "Q is not symmetric"

    Q += 1e-3*np.eye((ke+1)*n_contacts)
...
def objective(x: np.ndarray, Q: np.ndarray):
    return 0.5*x.T@Q@x
        
def objective_jac(x: np.ndarray, Q: np.ndarray):
    return Q@x

def eq_fn(x: np.ndarray, N: np.ndarray, D: np.ndarray):
...
def eq_jac(x: np.ndarray, N:np.ndarray, D:np.ndarray):
    G = G_T.T
    ns = ke+1
...
def ineq_fn(x:np.ndarray, A_ineq: np.ndarray):
    mu_EPDM = 1
    fn = x[::7]
    alpha = x.reshape(5,7)[:,1:7].reshape(30)

    return mu_EPDM*fn - A_ineq@alpha

def ineq_jac(x:np.ndarray, A_ineq: np.ndarray):
    mu_EPDM = 1
    ns = ke+1
    
    jac_mat = np.zeros((n_contacts, (ke+1)*n_contacts))
    for i in range(n_contacts):
        jac_mat[i,ns*i] = mu_EPDM
        jac_mat[i,ns*i+1:ns*i+ns] = -A_ineq[i,ke*i:ke*i+ke]
...
augmented_vectors_contact_pt = np.zeros(((ke+1)*n_contacts,3))
...
lb = np.zeros((ke+1)*n_contacts)
ub = 150*np.ones((ke+1)*n_contacts)
bounds = optimize.Bounds(lb,ub)

Q = build_Q(contact_vecs=augmented_vectors_contact_pt)
# print(np.linalg.eig(Q).eigenvalues)
# quit()
...
x0 = init_opt()
fn_opt = x0[::7]
alpha_opt = x0.reshape(5,7)[:,1:7].reshape(30)
print("Equality constraint residual at initial condition: ", G@N@fn_opt + G@D@alpha_opt + w_ext)
print("Inequality constraint residual at initial condition:", ineq_fn(x0, A_ineq=Aineq))
quit()

eq_cons = {'type': 'eq', 'fun': lambda x: eq_fn(x, N=N, D=D), 'jac': lambda x: eq_jac(x, N=N, D=D)}
ineq_cons = {'type': 'ineq', 'fun': lambda x: ineq_fn(x, A_ineq=Aineq), 'jac': lambda x: ineq_jac(x, A_ineq=Aineq)}

res = optimize.minimize(fun=lambda x: objective(x,Q=Q),x0=x0,constraints=[eq_cons,ineq_cons],method='SLSQP',jac=lambda x: objective_jac(x,Q=Q),options={'ftol': 1e-15, 'maxiter': 10000, 'disp': True}, bounds=bounds)
xopt = res.x
fn_opt = xopt[::7]
alpha_opt = xopt.reshape(5,7)[:,1:7].reshape(30)
print("Equality constraint residual at optimality: ", G@N@fn_opt + G@D@alpha_opt - w_ext)
print("Optimal fn:", fn_opt)
print("Optimal alpha:", alpha_opt)
print("Friction cone inequality values:", ineq_fn(xopt, A_ineq=Aineq))
