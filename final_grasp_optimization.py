...
for i in range(idx_sz):
    ...
    eq_cons = {'type': 'eq', 'fun': lambda f: eq_fn(f, cylinder_axis=rot_matrix[:,2], n_contacts=5), 'jac': lambda f: eq_jac(f, n_contacts=5)}
    nonlinear_constraint = optimize.NonlinearConstraint(lambda f: nonlin_cons_eq(f, n_contacts=5), 0.0, np.inf, jac=lambda f: nonlin_cons_jac(f, n_contacts=5))
    f0 = np.linalg.pinv(G) @ (-w_ext)
    # f0 = 1e-4*np.ones(15)
    f0[::3] = 1e-2
    print("Friction constraints: Initial condition feasibility", nonlin_cons_eq(f0, 5))
    res = optimize.minimize(fun=objective,x0=f0,constraints=[eq_cons, nonlinear_constraint],method='SLSQP',jac=objective_jac,bounds=bounds,options={'maxiter':5000, 'ftol': 1e-3, 'disp': True})
    fres = res.x
    print(res.message)
    print(fres)
