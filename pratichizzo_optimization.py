...
def partial_grasp_matrix(orientation: np.ndarray, ci: np.ndarray, obj_centroid: np.ndarray):
    ...
    R = block_diag(orientation, orientation)
...
def build_S(ke: np.int8, n_contacts: np.int8):
    S_list = np.zeros((n_contacts,3,ke))
    for i in range(n_contacts):
        Si = np.zeros((3,ke))
        for j in range(ke):
            idx = (2*j*np.pi)/ke
            Si[:,j] = np.array([1,mu_EPDM*np.cos(idx),mu_EPDM*np.sin(idx)]).T
        S_list[i,:,:] = Si
    
    S = block_diag(*(S_list[i,:,:] for i in range(n_contacts)))

    return S

def build_F(S: np.ndarray):
    sz = S.shape
    nc = int(sz[0]/3)
    ke = int(sz[1]/nc)
    F_list = np.zeros((nc,ke,3))
    for i in range(nc):
        Fi = np.zeros((ke,3))
        Si = S[3*i:3*i+3,ke*i:ke*i+ke]
        for j in range(ke):
            if(j==ke-1):
                Fi[j,:] = np.cross(Si[:,j],Si[:,0])
            else:
                Fi[j,:] = np.cross(Si[:,j],Si[:,j+1])
        F_list[i,:,:] = Fi
    
    F = block_diag(*(F_list[i,:,:] for i in range(nc)))
...
# Building the matrix of generators
S = build_S(ke=16,n_contacts=5)
F = build_F(S=S)
e = np.zeros((1,15))
e[0,::3] = 1
idx_ls = []
...
for i in range(146):
    ...
    F_col = np.ones((F.shape[0],1))

    ineq1 = np.hstack((F_col, -F))
    ineq2 = np.hstack((np.zeros((1,1)),e))
    Aub = np.vstack((ineq1,ineq2))

    bub = np.hstack((
        np.zeros(F.shape[0]),  # corresponding to ineq1 rows
        5  # corresponding to ineq2 (e lambda ≤ n_c)
    ))
    Aeq = np.hstack((np.zeros((G.shape[0],1)), G))
    beq = -w_ext

    lambda_bounds = [(None, None) for _ in range(15)]
    bounds = [(0, None),*lambda_bounds]

    res = optimize.linprog(c=d, A_ub=Aub, b_ub=bub, A_eq=Aeq, b_eq=beq, bounds=bounds, method='highs')
    if(res.fun is not None and res.fun != 0):
        print(res.message)
        print(res.fun)
        print(res.x)
        print(f"Iter {i}")
